  ...
  return (
    <div className="p4">
      ...
      <div className="table_header">
        <span className="p4" style={{ width: "10%" }}>
          Rank
        </span>
        <span className="p4" style={{ width: "15%" }}>
          User
        </span>
        <span className="p4" style={{ width: "25%" }}>
          Start Amount
        </span>
        <span className="p4" style={{ width: "25%" }}>
          End Amount
      ...
      {props.result.length > 0 &&
        props.result.map((el, idx) => (
          <div
            className={`table_body ${el.is_winner ? "winner-color" : ""}`}
            key={idx}
          >
            <span className="p4" style={{ width: "10%" }}>
              {idx + 1}
            ...
            <span className="p4" style={{ width: "25%" }}>
              {parseFloat(el.oldAmount).toFixed(2)}
            </span>
            <span className="p4" style={{ width: "25%" }}>
              {parseFloat(el.currentAmount).toFixed(2)}
            </span>
            <span className="p4" style={{ width: "25%" }}>{`${(
              parseFloat(el.oldAmount) - parseFloat(el.currentAmount)
            ).toFixed(2)} (${rateLostRaise(
